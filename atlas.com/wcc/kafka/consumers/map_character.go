...
func (mc *MapCharacter) processEvent(event MapCharacterEvent) {
	...
	mc.l.Printf("[INFO] processing MapCharacterEvent of type %s", event.Type)
	if event.Type == "ENTER" {
		mc.enter(*as, event)
...
func (mc *MapCharacter) enter(as mapleSession.MapleSession, event MapCharacterEvent) {
	...
	for _, m := range ms {
		mc.l.Printf("[INFO] spawning monster %d type %d for character %d", m.UniqueId(), m.MonsterId(), as.CharacterId())
