...
func HandleRangeAttackEvent() ChannelEventProcessor {
   return func(l logrus.FieldLogger, wid byte, cid byte, e interface{}) {
      if event, ok := e.(*rangeAttackEvent); ok {
         ...
         session.ForEachSessionInMap(event.WorldId, event.ChannelId, event.MapId, writeRangeAttack(event.CharacterId, event.SkillId, event.SkillLevel, event.Stance, event.AttackedAndDamaged, event.Damage, event.Speed, event.Direction, event.Display, event.Projectile))
      } else {
...
func writeRangeAttack(characterId uint32, skill uint32, skillLevel byte, stance byte, numberAttackedAndDamaged byte, damage map[uint32][]uint32, speed byte, direction byte, display byte, projectile uint32) func(s session.Model) {
