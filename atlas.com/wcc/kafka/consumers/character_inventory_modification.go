...
func HandleCharacterInventoryModificationEvent() ChannelEventProcessor {
	return func(l logrus.FieldLogger, span opentracing.Span, wid byte, cid byte, e interface{}) {
		if event, ok := e.(*characterInventoryModificationEvent); ok {
			...
			session.ForSessionByCharacterId(event.CharacterId, writeInventoryModification(l, span)(event))
		} else {
...
func writeInventoryModification(l logrus.FieldLogger, span opentracing.Span) func(event *characterInventoryModificationEvent) session.Operator {
	return func(event *characterInventoryModificationEvent) session.Operator {
			...
			for _, m := range event.Modifications {
				...
				if m.InventoryType == 1 {
					if m.Mode == 3 {
						...
						e, err := character.GetEquipItemForCharacter(l, span)(event.CharacterId, m.Position)
						if err != nil {
