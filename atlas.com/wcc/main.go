...
func main() {
	...
	go consumers.NewMonsterControl(l, context.Background()).Init(byte(wid), byte(cid))
	go consumers.NewMonsterEvent(l, context.Background(), byte(wid), byte(cid)).Init()
	go consumers.NewEnableActions(l, context.Background()).Init(byte(wid), byte(cid))
